...
async function fetchBreakingNews() {
  let response = await fetch(
    `http://api.mediastack.com/v1/news?access_key=d2e7cd704c760008100066b1a3258c3e&countries=gb&${date}&sources=bbc`,
    {
      method: "GET",
      redirect: "follow",
    }
...
async function fetchNewsType() {
  ...
  let response = await fetch(
    `http://content.guardianapis.com/search?${newsType}&api-key=${guardianApiKey}&show-fields=thumbnail`
  );
...
async function getRandomNasaBackground() {
  ...
  let nasaBackgrounds = [
    // Check over these images later on for sizing, colours etc.
    "2021-03-01",
    "2021-03-08",
    "2021-03-23",
    "2021-03-30",
  ...
  let randomNasaBackground =
    nasaBackgrounds[Math.floor(Math.random() * nasaBackgrounds.length)];
...
async function displayBreakingNews() {
  ...
  for (i = 0; i < 10; i++) {
    if (
      breakingNewsItems.data[i].title !== breakingNewsItems.data[i + 1].title
    ) {
